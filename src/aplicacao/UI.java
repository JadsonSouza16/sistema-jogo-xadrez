...
public class UI {
	...
	public static XadrezPosicao encontreXadrezPosicao(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro ao ler posição no xadrez. Valores preicsam está entre a1 e h8");
		}
	}

	public static void printPartida(PartidaXadrez partidaXadrez, List<XadrezPeca> capturada) {
	...
	public static void printTabuleiro(XadrezPeca[][] pecas, boolean[][] movimentosPossivel) {
		for (int i = 0; i < pecas.length; i++) {
			System.out.print((8 - i) + " ");
			for (int j = 0; j < pecas.length; j++) {
				printPeca(pecas[i][j], movimentosPossivel[i][j]);
			}
			System.out.println();
		}
		System.out.println("  a b c d e f g h");
